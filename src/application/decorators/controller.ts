...
export function Middleware(middlewareClass: new (...args: any[]) => MiddlewareHandler) {
    return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
        if (!middlewareClass.prototype.execute) {
            throw new FrameworkError('Middleware must implement execute()');
        }
        const middlewares: Array<new (...args: any[]) => MiddlewareHandler> = Reflect.getMetadata('middlewares', target, propertyKey) || [];
...
export function Middlewares(middlewareClasses: (new (...args: any[]) => MiddlewareHandler)[]) {
    return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
        ...
        const existingMiddlewares: Array<new (...args: any[]) => MiddlewareHandler> = 
